...
void setup() {
    ...
    std::string data_josn{"{\"head\":\"hello! I am DHT11!\"}"};
...
void OnMsg(const std::string& msg){
    ...
    switch (order) {
      ...
      case Order::SendData : {
        Serial.println("> Info : receive order [SendData] from APP");
        DealSendData();
        break;
      }
      default : {
...
void DealSendData(){
    ...
    data_josn += "\"tempe\":\"" + std::to_string(tempe) + "\",";
    data_josn += "\"moder\":\"" + std::to_string(humid) + "\"";
    data_josn += "}";
    client.publish(topic_data,data_josn.c_str());
}
