...
void setup() {
    ...
    //init SG90
    MG90S.attach(MG90SPIN,500,2500);

    // publish and subscribe
    std::string data_josn{"{\"head\":\"hello! I am MG90S!\"}"};
...
void OnMsg(const std::string& msg){
    ...
    switch (order) {
      ...
      case Order::FanState : {
        Serial.print("> Info : receive order [FanState] from APP");
        GetFanState();
        break;
      }
      case Order::AdjustFan : {
        int fan_gear = msg_json.Get<int>("fan_gear");
        Serial.print("> Info : receive order [AdjustFan] from APP, fan gear is ");
        Serial.println(fan_gear);
        DealAdjustFan(fan_gear);
        break;
      }
      default : {
...
void GetFanState(){
    ...
    data_josn += "\"head\":\"MG90S已经很累了!\",";
    data_josn += "\"fan_state\":\"" + std::to_string(fan_gear) + "\"}";
    data_josn += "}";
    client.publish(topic_data,data_josn.c_str());
}

void DealAdjustFan(int fan_gear){
    if((fan_gear == 0 && gear!=0)||(gear==0&&fan_gear!=0)){
      if(gear){
        while(gear>1){
          ReduceGear();
        }
